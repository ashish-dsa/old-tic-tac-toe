...
function computeNextMove(availableSquares) {
    ...
    if (randomValue == 0) {
        ...
        let randomSquare = availableSquares[Math.random() * availableSquares.length >> 0];
...
function showGameModal(winner) {
    ...
    let winnerModalContent = document.getElementsByClassName("winnerModalContent")[0];
    if(winner=='Player')
    winner=playerName;
    if (winner != '') {
        winnerModalContent.innerHTML = winner + ' ' + "Wins";
    ...
    setTimeout(
        () => {
            ...
            document.getElementsByClassName("gameEventModal")[0].style.display = "block";
...
function checkWinner(currentGrid, row, column, value) {
    ...
    //left
    if (patternRow(newGrid, row, value))
        return true;
    //right
    else if (patternColumn(newGrid, column, value))
        return true;
    //diagonalRight
    else if (patternDiagonalRight(newGrid, value))
        return true;
    //diagonalLeft
    else if (patternDiagonalLeft(newGrid, value))
        return true;
    else
        return false;
}
...
function setGridSize(gridSize) {
    if (!isNaN(gridSize) && gridSize.trim() != '')
...
function noMovesLeft(newGrid) {
    ...
    if (counter >= (GRID_LENGTH * GRID_LENGTH))
        return true;
    else
        return false;
}
